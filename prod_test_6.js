...
async function runProdTest() {
  ...
  try {
    ...
    const resOrder = await client.query(
      "INSERT INTO pedidos_app_orders (customer_name, customer_phone, crm_contact_id, source, total, delivery_type, payment_method) VALUES ('PROD TEST', '3999999999', " + idBsuid + ", 'MANUAL', 100, 'domicilio', 'efectivo') RETURNING id, crm_contact_id"
    );
    const order = resOrder.rows[0];

    assert.strictEqual(order.crm_contact_id, idPhone, 'Order reparented to phone contact');
