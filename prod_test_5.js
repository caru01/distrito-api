...
async function runProdTest() {
  ...
  try {
    ...
    // Create test phone contact with exactly 10 digits that normalize correctly
    const resPhone = await client.query("INSERT INTO pedidos_app_crm_contacts (display_name, normalized_phone, source, status) VALUES ('PROD TEST PHONE', '+573229999999', 'TEST', 'NUEVO_CONTACTO') RETURNING id");
    ...
    const queryStr = "INSERT INTO pedidos_app_orders (customer_name, customer_phone, crm_contact_id) VALUES ('PROD TEST', '3229999999', " + idBsuid + ") RETURNING id, crm_contact_id";
    const resOrder = await client.query(queryStr);
